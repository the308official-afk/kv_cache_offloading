...
def normalize_hints(hints: dict | None) -> NormalizedHints:
    ...
    latency_sensitivity = clamp(float(hints.get("latency_sensitivity", 0.5) or 0.5), 0.0, 1.0)
    program_id = str(hints.get("program_id", "unknown") or "unknown")
    context_type = str(hints.get("context_type", "generic") or "generic").lower()
...
def compute_worker_score(worker: WorkerSnapshot, hints: NormalizedHints) -> RoutingDecision:
    weights = dict(phase_weights(hints.agent_phase))
    if hints.latency_sensitivity > 0.0:
        weights["load_bias"] += 0.25 * hints.latency_sensitivity
    if "shared_prefix" in hints.context_type or "conversation" in hints.context_type:
        weights["cache_bias"] += 0.10
    total_bias = max(weights["cache_bias"] + weights["load_bias"], 1e-6)
    cache_bias = weights["cache_bias"] / total_bias
    load_bias = weights["load_bias"] / total_bias
    ...
    score = (
        cache_bias * cache_score
        + load_bias * load_score
        + 0.15 * priority_score
    )
    ...
    explanation = (
        ...
        f"latency={hints.latency_sensitivity:.3f} context={hints.context_type} "
        f"program={hints.program_id}"
    )
