...
SOURCE_REPO="${SOURCE_REPO:-${CACHE_PINNING_DYNAMO_SOURCE_REPO}}"
SOURCE_REF="${SOURCE_REF:-${CACHE_PINNING_DYNAMO_SOURCE_REF}}"
PULL_REF="${PULL_REF:-${CACHE_PINNING_DYNAMO_PULL_REF}}"
...
if [[ -d "${SOURCE_DIR}/.git" ]]; then
  if ! git -C "${SOURCE_DIR}" diff --quiet --ignore-submodules=all; then
    if [[ "${AUTO_RESET_DIRTY_SOURCE}" = "1" ]]; then
      echo "Resetting dirty cache-pinning Dynamo clone at ${SOURCE_DIR}"
      git -C "${SOURCE_DIR}" reset --hard >/dev/null
      git -C "${SOURCE_DIR}" clean -fd >/dev/null
    else
      echo "Existing cache-pinning Dynamo clone is dirty: ${SOURCE_DIR}" >&2
      exit 1
    fi
  fi
  echo "Updating cache-pinning Dynamo source clone at ${SOURCE_DIR}"
...
echo
echo "Cache-pinning Dynamo source is ready at:"
echo "  ${SOURCE_DIR}"
echo "Pinned ref:"
echo "  ${SOURCE_REF}"
