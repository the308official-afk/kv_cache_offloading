...
python3.11 experiments/scripts/lpx_decode_split/run_decode_sweep.py \
...
```text
experiments/reports/lpx_decode_split/results/
```
...
nsys export --force-overwrite true --type sqlite \
  --output experiments/raw/lpx_decode_split/profiles/<run>/<run>.sqlite \
  experiments/raw/lpx_decode_split/profiles/<run>/<run>.nsys-rep
```
...
python3.11 experiments/scripts/lpx_decode_split/analyze_nsys_sqlite.py \
  --sqlite experiments/raw/lpx_decode_split/profiles/<run>/<run>.sqlite \
  --worker-log experiments/raw/lpx_decode_split/profiles/<run>/dynamo-sglang-worker.full.log \
  --out-dir experiments/raw/lpx_decode_split/profiles/<run>/kernel_analysis
```
...
python3.11 experiments/scripts/lpx_decode_split/estimate_lpx_speedup.py \
  --classification-json experiments/raw/lpx_decode_split/profiles/<run>/kernel_analysis/kernel_classification.json \
