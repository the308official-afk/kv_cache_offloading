#!/usr/bin/env python3
"""Generate slide-ready SVG charts from KV retention microbenchmark_matrix.csv."""
...
def select_rows(rows: list[dict[str, str]], *, part: str) -> list[dict[str, str]]:
    return [row for row in rows if row.get("part") == part]
...
def build_probe_charts(rows: list[dict[str, str]], out_dir: Path) -> list[dict[str, str]]:
    probe_rows = select_rows(rows, part="probe")
    if not probe_rows:
        return []

    labels = [label_for_row(row) for row in probe_rows]
    latency_values = [parse_float(row.get("replay_ms")) for row in probe_rows]
    cached_values = [parse_float(row.get("replay_cached")) or 0.0 for row in probe_rows]
    ...
    latency_svg = build_bar_chart_svg(
        ...
        subtitle="Probe replay latency by arm",
        labels=labels,
    ...
    cached_svg = build_bar_chart_svg(
        title="KV Retention Replay Cached Tokens",
        subtitle="Probe replay cached tokens by arm",
        labels=labels,
    ...
    return [
        {"chart_key": "replay_latency", "path": str(latency_path), "part": "probe"},
        {"chart_key": "replay_cached_tokens", "path": str(cached_path), "part": "probe"},
    ]


def build_sweep_charts(rows: list[dict[str, str]], out_dir: Path) -> list[dict[str, str]]:
    sweep_rows = select_rows(rows, part="sweep")
    if not sweep_rows:
        return []
    ...
    for row in sweep_rows:
        key = (row.get("arm", ""), row.get("hint_profile", "") or row.get("cache_control", "") or row.get("kv_tier", ""))
        if key not in seen:
            seen.add(key)
            series_keys.append(key)

    palette = ["#2563eb", "#16a34a", "#f59e0b", "#7c3aed", "#dc2626", "#0891b2"]
    color_map = {key: palette[idx % len(palette)] for idx, key in enumerate(series_keys)}

    def series_for(metric: str, *, warm_metric: bool = False) -> list[dict[str, object]]:
        ...
        for arm, secondary in series_keys:
            ...
            for distractor in distractors:
                ...
                if not match:
                    ...
                    values.append(1.0 if boolish(match.get(metric)) else 0.0)
                else:
                    values.append(parse_float(match.get(metric)))
            built.append(
                {
                    "label": f"{arm}: {secondary}" if secondary else arm,
    ...
    cached_svg = build_line_chart_svg(
        ...
        x_values=distractors,
        series=series_for("replay_cached"),
        y_label="Replay Cached Tokens",
    )
    survival_svg = build_line_chart_svg(
        title="KV Retention Survival Curve",
        subtitle="Warm replay status versus distractor count",
        x_values=distractors,
...
def main() -> int:
    ...
    if sweep_rows:
        ...
        manifest["generated_files"].extend(build_probe_charts(rows, out_dir))
