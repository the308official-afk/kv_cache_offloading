...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--control-cache-control-profile", default="off")
...
def compact_arm_label(row: dict[str, Any]) -> str:
    return "control" if str(row.get("is_control", "")).lower() == "true" else "protected"


def compact_matrix_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "status": row.get("sweep_status", ""),
        "sweep_id": row.get("retention_sweep_id", ""),
        "model": row.get("model", ""),
        "kv_tier": row.get("kv_tier_mode", ""),
        "arm": compact_arm_label(row),
        "hint_profile": row.get("hint_profile", ""),
        "protected_cache": row.get("protected_cache_control_profile", ""),
        "distractors": row.get("distractor_count", ""),
        "first_status": row.get("a_first_status", ""),
        "replay_status": row.get("a_replay_status", ""),
        "first_ms": row.get("a_first_latency_ms", ""),
        "replay_ms": row.get("a_replay_latency_ms", ""),
        "replay_delta_ms": row.get("a_replay_latency_delta_ms", ""),
        "replay_speedup": row.get("a_replay_speedup_ratio", ""),
        "replay_cached": row.get("a_replay_cached_tokens", ""),
        "replay_reuse": row.get("a_replay_cache_reuse_ratio", ""),
        "survived": row.get("survived_effective", ""),
        "survival_source": row.get("effective_survival_source", ""),
        "reuse_status": row.get("reuse_signal", ""),
        "req_cache_status": row.get("request_cache_control_status", ""),
        "req_cache_values": row.get("request_cache_control_values", ""),
        "worker_cache_status": row.get("worker_cache_control_status", ""),
        "worker_cache_values": row.get("worker_cache_control_values", ""),
        "replay_evicts": row.get("a_replay_sglang_cache_evict_events", ""),
        "replay_evict_cache": row.get("a_replay_sglang_evict_cache_control_values", ""),
        "replay_evict_cache_match": row.get("a_replay_sglang_evict_cache_control_match", ""),
        "replay_evict_status": row.get("a_replay_sglang_evict_identity_status", ""),
        "effect_status": row.get("hint_runtime_effect_status", ""),
    }
...
def derived_row(
    *,
    ...
    control_cache_control_profile: str,
    match_event_min: int,
    ...
    cache_control = summarize_request_text(request_rows, field="cache_control_profile")
    ...
    protected_cache_control_profile = summary.get("protected_cache_control_profile", "")
    distractor_cache_control_profile = summary.get("distractor_cache_control_profile", "")
    ...
    return {
        ...
        "protected_cache_control_profile": protected_cache_control_profile,
        "distractor_cache_control_profile": distractor_cache_control_profile,
        "request_cache_control_status": cache_control["status"],
        "request_cache_control_values": cache_control["values"],
        "is_control": str(
            hint_profile == control_hint_profile
            and protected_cache_control_profile == control_cache_control_profile
        ).lower(),
        ...
        "a_replay_sglang_cache_evict_events": summary.get("a_replay_sglang_cache_evict_events", ""),
        ...
        "a_replay_sglang_evict_cache_control_values": summary.get("a_replay_sglang_evict_cache_control_values", ""),
        "a_replay_sglang_evict_hint_profiles": summary.get("a_replay_sglang_evict_hint_profiles", ""),
        "a_replay_sglang_evict_cache_control_match": summary.get("a_replay_sglang_evict_cache_control_match", ""),
        "a_replay_sglang_evict_hint_profile_match": summary.get("a_replay_sglang_evict_hint_profile_match", ""),
        "a_replay_sglang_evict_identity_status": summary.get("a_replay_sglang_evict_identity_status", ""),
...
def annotate_hint_runtime_effect(
    rows: list[dict[str, Any]],
    *,
    control_hint_profile: str,
    control_cache_control_profile: str,
) -> None:
    grouped: dict[tuple[str, str, str, int], dict[tuple[str, str], dict[str, Any]]] = {}
    for row in rows:
        key = (
            str(row["model"]),
        ...
        arm_key = (
            str(row["hint_profile"]),
            str(row.get("protected_cache_control_profile", "")),
        )
        grouped.setdefault(key, {})[arm_key] = row

    for row in rows:
        hint_profile = str(row["hint_profile"])
        cache_control_profile = str(row.get("protected_cache_control_profile", ""))
        if (
            hint_profile == control_hint_profile
            and cache_control_profile == control_cache_control_profile
        ):
        ...
        control_row = grouped.get(key, {}).get((control_hint_profile, control_cache_control_profile))
...
def build_comparison_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        arm_key = (row["hint_profile"], row.get("protected_cache_control_profile", ""))
        grouped.setdefault(key, {}).setdefault(arm_key, []).append(row)

    out: list[dict[str, Any]] = []
    for (model, retention_attribution_mode, kv_tier_mode), by_profile in sorted(grouped.items()):
        control_rows = sorted(
            by_profile.get((control_hint_profile, control_cache_control_profile), []),
        ...
        for (hint_profile, protected_cache_control_profile), profile_rows in sorted(by_profile.items()):
            if (
                hint_profile == control_hint_profile
                and protected_cache_control_profile == control_cache_control_profile
            ):
...
def write_summary_md(
    ...
    *,
    ...
    cache_profiles = sorted({str(row.get("protected_cache_control_profile", "")) for row in matrix_rows if str(row.get("protected_cache_control_profile", ""))})
    lines = [
        ...
        f"- Control cache-control profile: {control_cache_control_profile}",
    ...
    if not comparison_rows:
        ...
        for row in comparison_rows:
            lines.extend(
                [
                    f"- `{row['model']}` / `{row['retention_attribution_mode']}` / `{row['kv_tier_mode']}` / `{row['protected_hint_profile']}` / `{row['protected_cache_control_profile']}`:",
...
def main() -> int:
        ...
        for summary in summaries:
            matrix_rows.append(
                derived_row(
                    ...
                    control_cache_control_profile=args.control_cache_control_profile,
    ...
    annotate_hint_runtime_effect(
        matrix_rows,
        control_hint_profile=args.control_hint_profile,
        control_cache_control_profile=args.control_cache_control_profile,
    )

    compact_matrix_rows = [compact_matrix_row(row) for row in matrix_rows]
    compact_matrix_fields = [
        "status",
        "sweep_id",
        "model",
        "kv_tier",
    ...
    write_csv(Path(args.out_matrix), compact_matrix_rows, compact_matrix_fields)
