...
- were request hints merely sent, or is there evidence the runtime saw or used them?

## Step 9: Hint alignment
AgentBench can attach simple runtime hints to the request.

For example:
- `hint_probe_id=...::hint_probe` is a bright tracking label for one run
- `priority=5` means this request is important
- `reuse_likelihood=0.9` means cache reuse may matter
- `expected_output_tokens=2048` means the response may need that much output room
- `agent_phase=baseline_execution` labels the phase of the run

The hint alignment report keeps these claims separate:

- **sent**: AgentBench created the hint
- **carried forward**: the request wrapper included it in the request payload
- **observed**: runtime logs showed the hint
- **behavior-supported**: runtime behavior was consistent with the hint
- **not proven**: the report does not have enough evidence to say the hint was used

This distinction matters because a hint can be attached to a request without proving that the SGLang worker acted on it.

In simple terms:
- hint alignment says whether the delivery note traveled with the request
- it does not assume the worker obeyed the note unless runtime evidence supports that claim
- the probe id lets us see where the delivery note disappears, if it disappears
